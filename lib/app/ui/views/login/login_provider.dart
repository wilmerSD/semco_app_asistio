...
class LoginProvider with ChangeNotifier {
  int counter = 0;
  TextEditingController ctrlUserText = TextEditingController(
    text: '',
  ); //Jose.sanchez
  ...
  void goToHome(BuildContext context) {
    Navigator.pushReplacement(
      ...
      ), // Reemplaza con tu pantalla Home
    );
  }
  ...
  void authentication(context) async {
    ...
    try {
      final response = await userRepository.postLogin(
        RequestLoginModel(
          pUsername: ctrlUserText.text,
          pPassword: ctrlPasswordText.text,
        ),
      );
      if (!response.success) {
        CustomSnackbar.showSnackBarSuccess(
      ...
      if (rememberPass) {
        await secureStorage.write(
          key: Keys.kUserName,
          value: ctrlUserText.text,
        );
        await secureStorage.write(
          key: Keys.kPassword,
          value: ctrlPasswordText.text,
        );
      }

      await secureStorage.write(
        key: Keys.kIdUser,
        value: responseData.personalId,
      );
      await secureStorage.write(
        key: Keys.kIdRole,
        value: responseData.personalRol,
      );
      await secureStorage.write(
        key: Keys.kNameUser,
        value: responseData.personalNombreCompleto,
      );
      await secureStorage.write(
        key: Keys.kEmail,
        value: responseData.personalCorreo,
      );
      await secureStorage.write(
        key: Keys.kSaveCredential,
        value: rememberPass.toString(),
      );
      goToHome(context);
    } catch (e) {
      CustomSnackbar.showSnackBarSuccess(
